...
public class ApiMemberController {
    ...
    public ResponseEntity<?> cancelCourse(Model model
        ...
        if (detail == null) { //수강정보가 없는 경우
            ResponseResult responseResult = new ResponseResult(false, "수강 신청 정보가 존재하지 않습니다.");
            return ResponseEntity.ok().body(responseResult);
        }

        if (userId == null || !userId.equals(detail.getUserId())) { //내 수강신청 정보가 아닌 경우
            ResponseResult responseResult = new ResponseResult(false, "본인의 수강 신청 정보만 취소 가능합니다.");
            return ResponseEntity.ok().body(responseResult);
        }
